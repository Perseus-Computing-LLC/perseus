...
# ── Optional: Vault ────────────────────────────────────────────────────────
# Database path for Vault (default: vault.db in working directory)
# VAULT_DB_PATH=~/.perseus/vault/vault.db
ENVEOF
...
# Vault memory backend (optional — uncomment when vault is installed)
# vault:
#   enabled: true
#   transport: "stdio"
#   command: ["vault", "--db", "~/.perseus/vault/vault.db"]
#   timeout_s: 10.0
