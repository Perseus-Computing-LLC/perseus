...
<section class="section reproduce-section"><div class="wrap install-layout"><div><p class="kicker">Reproduce</p><h2>Start from the report, not a screenshot.</h2><p>The repository keeps reproducible commands beside public artifacts where available. The current paired LongMemEval report is internal-only and is not published; use the claim registry for its method and status.</p></div><div>""" + command("git clone https://github.com/Perseus-Computing-LLC/perseus-vault.git\ncd perseus-vault\npython benchmark/temporal/gauntlet.py") + """</div></div></section>
"""
...
<section class="section"><div class="wrap install-layout"><div><p class="kicker">Run the published product</p><h2>Generate a complete artifact in your own workspace.</h2><p>The real loop runs locally: author sources, render the briefing, inspect the output, then give that file to the assistant you already use.</p></div><div>""" + command("pip install perseus-ctx==1.0.26\nperseus quickstart\nperseus render .perseus/context.md -o AGENTS.md") + """</div></div></section>
"""
