...
VAULT_DEMO = "https://vault-demo.perseus.observer/"
MEMCONFLICT_REPORT = "https://perseus-computing-llc.github.io/hermes-memconflict/report/"
...
NAV = [
    ...
    ("proof", "/benchmarks/", "Evidence"),
    ("defense", "/government/", "Government"),
    ("docs", "/docs/", "Docs"),
    ("demo", VAULT_DEMO, "Try Vault demo ↗"),
]
...
def nav_link(key: str, href: str, label: str, current: str) -> str:
    ...
    external_attr = ' rel="noopener"' if href.startswith("http") else ""
    return f'<a href="{href}"{current_attr}{external_attr}>{label}</a>'


def header(current: str, cta_href: str = "/docs/", cta_label: str = "Start locally") -> str:
    demo_is_hero_cta = cta_href.startswith(VAULT_DEMO)
    visible_nav = [(key, href, label) for key, href, label in NAV if not (key == "demo" and demo_is_hero_cta)]
    links = "".join(nav_link(key, href, label, current) for key, href, label in visible_nav)
    def mobile_link(href: str, label: str) -> str:
        rel = ' rel="noopener"' if href.startswith("http") else ""
        return f'<a href="{href}"{rel}>{label}</a>'
    mobile = "".join(mobile_link(href, label) for _, href, label in visible_nav)
    cta_rel = ' rel="noopener"' if cta_href.startswith("http") else ""
    cta_markup = "" if demo_is_hero_cta else f'<a class="button button-primary nav-cta" href="{cta_href}"{cta_rel}>{cta_label}</a>'
    mobile_cta_markup = "" if demo_is_hero_cta else f'<a href="{cta_href}"{cta_rel}>{cta_label}</a>'
    return f"""
<header class="site-header" data-site-nav>
  <div class="wrap nav-row">
    ...
    <div class="nav-actions">
      <button class="icon-button theme-toggle" type="button" data-theme-toggle aria-label="Use dark theme"><span aria-hidden="true">Theme</span></button>
{cta_markup}
  ...
  <nav class="mobile-nav" id="mobile-navigation" data-mobile-menu aria-label="Mobile">{mobile}{mobile_cta_markup}</nav>
</header>"""
...
<footer class="site-footer">
  <div class="wrap footer-grid">
    ...
    <div><h2>Read more</h2><a href="/security/">Security boundary</a><a href="/benchmarks/">Results and methods</a><a href="{MEMCONFLICT_REPORT}" rel="noopener">MemConflict comparison ↗</a><a href="/docs/">Install and source</a></div>
...
<section class="hero home-hero">
  <div class="wrap hero-layout">
    <div class="hero-copy">
      <p class="kicker">Open-source platform for AI-assisted work</p>
      <h1>Give every task the context it needs.</h1>
      <p class="lead">Perseus adds current workspace context, durable memory, and reviewable records around the model or system you already use.</p>
      <div class="actions"><a class="button button-primary" href="{VAULT_DEMO}#try" rel="noopener">Try Vault demo ↗</a><a class="button button-secondary" href="/government/">See DoD fit →</a></div>
      <p class="boundary-note"><strong>Local by default.</strong> You choose the model, data path, deployment, and permissions.</p>
    </div>
    <div class="hero-proof">
      <div class="proof-header"><span>Product measurements</span><small>Company-run A/B and scale tests</small></div>
      <div class="stat-grid">
        <article class="stat-card"><strong>52.63%</strong><span>fewer full-request prompt tokens</span><small>Naive full-file assembly vs. shipped-default render · cold cache · context + 14 prompts · named repo corpus</small><a class="text-link" href="https://github.com/Perseus-Computing-LLC/perseus/blob/main/benchmark/tokenab/report.json" rel="noopener">Read the token report ↗</a></article>
        <article class="stat-card"><strong>99.8%</strong><span>session recall@10</span><small>LongMemEval-S current-main · retrieval only · 500 instances</small><a class="text-link" href="https://github.com/Perseus-Computing-LLC/perseus-vault/blob/main/benchmark/longmemeval/report-currentmain-2026-08-16.json" rel="noopener">Retrieval report ↗</a></article>
        <article class="stat-card wide"><strong>40 writes/s</strong><span>durable write throughput</span><small>Measured at 100K entities on named hardware · signed scale report · not an SLA</small><a class="text-link" href="https://github.com/Perseus-Computing-LLC/perseus-vault/blob/main/benchmark/scale/report.json" rel="noopener">Read the scale report ↗</a></article>
      </div>
      <p class="stat-footnote">Token reduction measures content assembly, not provider billing or dollar savings. Retrieval is retrieval-only; throughput is not an SLA.</p>
    </div>
  </div>
</section>
<section class="section route-section" id="system">
  <div class="wrap">
    <div class="section-heading"><div><p class="kicker">One platform, three jobs</p><h2>Keep work moving.</h2></div><p>Use one component or all three. Each has a small, reviewable job.</p></div>
    <div class="route-list">
      <a href="/context-engine/"><span>01 / now</span><h3>Perseus Context Engine</h3><p>Build one briefing from the sources you choose.</p><b>See Context Engine →</b></a>
      <a href="/vault/"><span>02 / later</span><h3>Perseus Vault</h3><p>Carry decisions and time-valid facts into the next task.</p><b>See Vault →</b></a>
      <a href="/ledger/"><span>03 / review</span><h3>Perseus Ledger</h3><p>Record supplied events and references for later review.</p><b>See Ledger →</b></a>
    </div>
  </div>
</section>
<section class="section audience-section">
  <div class="wrap"><div class="section-heading"><div><p class="kicker">Choose a path</p><h2>Start where you are.</h2></div><p>See the product, the evidence, or the bounded Government workshare.</p></div><div class="link-grid">
    <a href="/government/"><span>For DoD programs and primes</span><h3>See where Perseus fits</h3><p>An enabling component beside the mission system.</p><b>Government overview →</b></a>
    <a href="{VAULT_DEMO}#try" rel="noopener"><span>For a hands-on look</span><h3>Try Vault demo ↗</h3><p>Save a decision, find it later, and prepare a short summary.</p><b>Open the demo</b></a>
    <a href="/benchmarks/"><span>For technical evaluators</span><h3>Read the evidence</h3><p>Numbers, methods, controls, and limits in one place.</p><b>See the methods →</b></a>
  </div></div>
</section>
<section class="section evidence-section"><div class="wrap proof-layout"><div><p class="kicker">Open evidence</p><h2>See the result, then the method.</h2><p>MemConflict tests changing facts, planted contradictions, and conditional preferences across eight self-hosted configurations.</p><div class="actions"><a class="button button-secondary" href="/benchmarks/">All methods</a><a class="text-link" href="{MEMCONFLICT_REPORT}" rel="noopener">Full comparison ↗</a></div></div><div class="report-card"><span>MemConflict / tested Perseus Vault configuration</span><strong>0.555</strong><p>Macro score in the self-run replication.</p><p class="card-note"><b>18 wrong</b> across 3,750 questions; 1,378 were blank or abstaining (36.7%). Wrong and blank outcomes are counted separately.</p><a href="{MEMCONFLICT_REPORT}" rel="noopener">Read the eight-provider report ↗</a></div></div></section>
<section class="section boundary-section">
  <div class="wrap boundary-layout">
    <div><p class="kicker">Deployment</p><h2>Run it where the work is.</h2><p>Local, on premises, private cloud, or disconnected. The program chooses the data, keys, network, and permissions.</p><a class="text-link" href="/security/">See the security boundary →</a></div>
    <dl class="boundary-table"><div><dt>Data</dt><dd>Stays in the environment you select for the local path.</dd></div><div><dt>Keys</dt><dd>You supply and keep the keys used for encryption and services.</dd></div><div><dt>Authority</dt><dd>The model and host keep their existing permissions. Perseus does not add mission-system authority.</dd></div></dl>
  </div>
</section>
<section class="section next-section"><div class="wrap next-bar"><div><p class="kicker">Next step</p><h2>See it work or start locally.</h2></div><div class="actions"><a class="button button-primary" href="{VAULT_DEMO}#try" rel="noopener">Try Vault demo ↗</a><a class="button button-secondary" href="/docs/">Start locally</a><a class="button button-secondary" href="/government/">Talk through a workflow</a></div></div></section>
"""
...
VAULT = f"""
<section class="product-hero vault-hero"><div class="wrap product-hero-grid"><div><p class="kicker">Perseus Vault</p><h1>Memory for the next task.</h1><p class="lead">Save decisions, corrections, and dated facts locally. Vault retrieves the part that fits when work starts again.</p><div class="actions"><a class="button button-primary" href="{VAULT_DEMO}#try" rel="noopener">Try Vault demo ↗</a><a class="button button-secondary" href="#install">Install Vault</a><a class="text-link" href="https://github.com/Perseus-Computing-LLC/perseus-vault/releases/tag/v2.23.2" rel="noopener">Release v2.23.2 ↗</a></div></div><div class="memory-timeline" role="list" aria-label="Memory lifecycle"><div role="listitem"><span>Save</span><p>Store a decision and when it applies.</p></div><div role="listitem"><span>Find</span><p>Return the useful part to a later task.</p></div><div role="listitem"><span>Update</span><p>Replace an old fact without erasing its history.</p></div><div role="listitem"><span>Archive</span><p>Keep or remove it as you decide.</p></div></div></div></section>
<section class="section"><div class="wrap product-job"><div><p class="kicker">How it works</p><h2>Remember what changed.</h2></div><div class="principle-list"><div><b>Dates</b><p>Keep track of when a fact was true.</p></div><div><b>Selection</b><p>Return a useful subset instead of the whole history.</p></div><div><b>History</b><p>Update or archive without erasing the record.</p></div></div></div></section>
<section class="section" id="install"><div class="wrap install-layout"><div><p class="kicker">Run it locally</p><h2>Install it, then connect your host.</h2><p>This command downloads the current x86_64 Linux release and checks its SHA-256 before extraction. Use the release page for other platforms. Start with local stdio before enabling a network connection.</p></div>""" + command(VAULT_LINUX_INSTALL) + f"""<p class="source-note"><a href="https://github.com/Perseus-Computing-LLC/perseus-vault/releases/tag/v2.23.2">See release assets and provenance ↗</a></p></div></div></section>
...
<section class="section next-section"><div class="wrap next-bar"><div><p class="kicker">Next step</p><h2>Try the demo or connect a host.</h2></div><div class="actions"><a class="button button-primary" href="{VAULT_DEMO}#try" rel="noopener">Try Vault demo ↗</a><a class="button button-secondary" href="/vault/mcp-reference/">Open the API entry</a><a class="button button-secondary" href="/docs/#integrations">Connect a host</a></div></div></section>
"""
...
<section class="section"><div class="wrap"><div class="method-head"><div><span>Accepted frozen-default series</span><h2>LongMemEval-S, three runs</h2></div><div class="method-result"><strong>1,213 / 1,500</strong><span>80.9% Perseus Vault mean</span></div></div><div class="method-grid"><dl><div><dt>Protocol</dt><dd>Official-CoT prompt, pinned answerer and official per-type judge, temperature 0</dd></div><div><dt>Retrieval and context</dt><dd>Hybrid k=10 with bundled ONNX embeddings; full context assembly, 32,768-token budget</dd></div><div><dt>Controls</dt><dd>Matched full-context control: 66.9% pooled; gold-session oracle: 90.8% pooled</dd></div></dl><div class="limitation"><b>What this means</b><p>Three accepted company-run replicates on the same 500-question split. The source revisions and separately named correction histories are recorded in the public report. This is not a customer, production, independent-holdout, or cross-model claim.</p><a href="https://github.com/Perseus-Computing-LLC/perseus-vault/blob/main/benchmark/longmemeval/qa_report_cot_frozen_default_series_20260828.json">Read the public series report ↗</a></div></div><div class="table-wrap"><table class="data-table"><thead><tr><th>Run</th><th>Stateless</th><th>Full context</th><th>Perseus Vault</th><th>Oracle</th></tr></thead><tbody><tr><th>1</th><td>45/500 · 9.0%</td><td>334/500 · 66.8%</td><td>401/500 · 80.2%</td><td>451/500 · 90.2%</td></tr><tr><th>2</th><td>47/500 · 9.4%</td><td>333/500 · 66.6%</td><td>403/500 · 80.6%</td><td>458/500 · 91.6%</td></tr><tr><th>3</th><td>45/500 · 9.0%</td><td>337/500 · 67.4%</td><td>409/500 · 81.8%</td><td>453/500 · 90.6%</td></tr><tr><th>pooled / mean</th><td>137/1,500 · 9.1%</td><td>1,004/1,500 · 66.9%</td><td><strong>1,213/1,500 · 80.9%</strong></td><td>1,362/1,500 · 90.8%</td></tr></tbody></table></div><p class="source-note">Public report content SHA-256: <code>ae6547565cafa8eeb0b7750380ea17b89e5b1d378a757a841b9077f393958955</code></p></div></section>
<section class="section"><div class="wrap"><div class="section-heading"><p class="kicker">Vault scorecard</p><h2>Where the series succeeds and misses.</h2><p>These are pooled counts across the three accepted runs. The full report contains the same breakdown for every arm.</p></div><div class="table-wrap"><table class="data-table"><thead><tr><th>Question type</th><th>Correct</th><th>Accuracy</th></tr></thead><tbody><tr><th>knowledge-update</th><td>193 / 234</td><td>82.5%</td></tr><tr><th>multi-session</th><td>286 / 399</td><td>71.7%</td></tr><tr><th>single-session-assistant</th><td>168 / 168</td><td>100.0%</td></tr><tr><th>single-session-preference</th><td>53 / 90</td><td>58.9%</td></tr><tr><th>single-session-user</th><td>206 / 210</td><td>98.1%</td></tr><tr><th>temporal-reasoning</th><td>307 / 399</td><td>76.9%</td></tr></tbody></table></div></div></section>
<section class="section"><div class="wrap"><div class="method-head"><div><span>Separate experimental result</span><h2>Evidence-structured context</h2></div><div class="method-result"><strong>410 / 500</strong><span>82.0% candidate</span></div></div><div class="method-grid"><dl><div><dt>Matched control</dt><dd>416/500 (83.2%) full context</dd></div><div><dt>Difference</dt><dd>-1.2 percentage points</dd></div><div><dt>Status</dt><dd>Report is internal-only; preregistered success rule failed; not blended into the frozen-default series</dd></div></dl><div class="limitation"><b>Boundary</b><p>This is a separate company-run paired confirmation. It is not a superiority, independent-holdout, customer, deployment, or production-authorization claim.</p><a href="https://github.com/Perseus-Computing-LLC/perseus/blob/main/claims.json">See the canonical claim registry ↗</a></div></div></div></section>
<section class="section measurement-section"><div class="wrap"><div class="section-heading"><p class="kicker">Other measurements</p><h2>Keep unlike tests separate.</h2></div><div class="measurement-list"><article><span>Quality</span><h3>Frozen-default answer accuracy</h3><p>Three accepted LongMemEval-S runs using the official-CoT prompt. The public series report includes per-run scores, pooled counters, category breakdowns, and hash commitments.</p><a href="https://github.com/Perseus-Computing-LLC/perseus-vault/blob/main/benchmark/longmemeval/qa_report_cot_frozen_default_series_20260828.json">Read the series report →</a></article><article><span>Retrieval</span><h3>Session recall</h3><p>99.8% company-run session-level recall@10 on the current-main 500-instance LongMemEval split. Hybrid recall@1 83.2%, recall@3 96.6%, recall@5 98.8%, and MRR 0.8949. Offline and judge-free; retrieval only, not answer accuracy or customer performance.</p><a href="https://github.com/Perseus-Computing-LLC/perseus-vault/blob/main/benchmark/longmemeval/report-currentmain-2026-08-16.json">Signed report →</a></article><article><span>Correctness</span><h3>Rebuilding past state</h3><p>13/13 company-run offline fixture scenarios across the temporal gauntlet and BEAM corpus tiers. No comparative control arm. This tests reconstruction for those fixtures, not real-world model quality or production validation.</p><a href="https://github.com/Perseus-Computing-LLC/perseus-vault/blob/main/benchmark/temporal/gauntlet_report.json">Temporal report →</a></article><article><span>Operations</span><h3>Latency and durable writes</h3><p>Signed scale report over the real binary and named hardware. Medians stay paired with tail latency.</p><a href="https://github.com/Perseus-Computing-LLC/perseus-vault/blob/main/benchmark/scale/report.json">Scale report →</a></article><article><span>Protocol replication</span><h3>MemConflict</h3><p>Self-run third-party protocol replication, not official inclusion on the benchmark author's page.</p><a href="/benchmarks/memconflict/">Read the replication →</a></article></div></div></section>
<section class="section reproduce-section"><div class="wrap install-layout"><div><p class="kicker">Reproduce</p><h2>Start with the report.</h2><p>The three-run QA summary is public and sanitized. Raw prompts, conversations, hypotheses, provider responses, credentials, and authorization headers are not published.</p></div><div>""" + command("git clone https://github.com/Perseus-Computing-LLC/perseus-vault.git\ncd perseus-vault\npython benchmark/longmemeval/qa.py --mock-llm --limit 5\npython benchmark/longmemeval/run.py") + """</div></div></section>
"""

# The public results route is a short index; detailed methods stay in the linked reports.
BENCHMARKS = f"""
<section class="page-intro results-intro"><div class="wrap intro-grid"><div><p class="kicker">Results and methods</p><h1>Numbers first. Conditions beside them.</h1></div><div><p class="lead">These are company-run reference measurements. Each card names the test, the scope, and the limit.</p><div class="actions"><a class="button button-primary" href="{MEMCONFLICT_REPORT}" rel="noopener">Full MemConflict comparison ↗</a><a class="text-link" href="/government/">DoD context →</a></div></div></div></section>
<section class="section results-section"><div class="wrap"><div class="results-grid"><article class="result-card featured"><span>Quality / LongMemEval-S</span><strong>80.9%</strong><h2>three-run mean</h2><p><b>1,213 / 1,500</b> correct on the 500-question split.</p><small>Three company-run runs, 500 questions each. No customer, production, independent-holdout, or cross-model claim.</small><a href="https://github.com/Perseus-Computing-LLC/perseus-vault/blob/main/benchmark/longmemeval/qa_report_cot_frozen_default_series_20260828.json">Read the series report ↗</a></article><article class="result-card risk"><span>Conflict handling / MemConflict</span><strong>18</strong><h2>wrong answers out of 3,750 questions</h2><p>In the tested Perseus Vault configuration.</p><small>1,378 / 3,750 blank or abstaining (36.7%); wrong counted separately. Self-run comparison, not operational validation.</small><a href="{MEMCONFLICT_REPORT}" rel="noopener">Read the full comparison ↗</a></article><article class="result-card"><span>Retrieval</span><strong>99.8%</strong><h2>session recall@10</h2><p>Company-run retrieval on the 500-instance LongMemEval split.</p><small>Retrieval only. No answer-quality control arm.</small><a href="https://github.com/Perseus-Computing-LLC/perseus-vault/blob/main/benchmark/longmemeval/report-currentmain-2026-08-16.json">Read the signed report ↗</a></article><article class="result-card"><span>Efficiency / Token A/B</span><strong>52.63%</strong><h2>fewer full-request prompt tokens</h2><p>Naive full-file assembly versus the shipped-default render on a named repo corpus.</p><small>Cold cache, context + 14 prompts, five fixture documents. Content assembly only; not provider billing or dollar savings.</small><a href="https://github.com/Perseus-Computing-LLC/perseus/blob/main/benchmark/tokenab/report.json" rel="noopener">Read the token report ↗</a></article><article class="result-card"><span>Operations</span><strong>40 writes/s</strong><h2>durable write throughput</h2><p>Signed scale report at 100K entities on named hardware.</p><small>Throughput only. Not customer capacity, deployment performance, or an SLA.</small><a href="https://github.com/Perseus-Computing-LLC/perseus-vault/blob/main/benchmark/scale/report.json">Read the scale report ↗</a></article></div></div></section>
<p class="source-note">The separate 82.0% evidence-structured confirmation remains internal-only and is not blended into the public three-run series.</p>
<section class="section results-limits"><div class="wrap"><div class="section-heading"><div><p class="kicker">Read before using</p><h2>What these numbers do not say.</h2></div><p>They do not establish a mission result. They show what was measured in named company-run tests.</p></div><div class="limit-grid"><article><span>Not a certification</span><p>No ATO/cATO, CMMC certification, classified-data authority, or mission-system authority is claimed.</p></article><article><span>Not a customer outcome</span><p>No customer, operational deployment, or Government authorization is represented by these measurements.</p></article><article><span>Not a dollar claim</span><p>Weighted token workload and context reduction are not provider billing, cost savings, or a service-level promise.</p></article></div></div></section>
<section class="section"><div class="wrap source-register"><div><p class="kicker">Detailed reports</p><h2>Follow the evidence.</h2></div><div><a href="https://github.com/Perseus-Computing-LLC/perseus-vault/blob/main/benchmark/longmemeval/qa_report_cot_frozen_default_series_20260828.json">LongMemEval-S series report ↗</a><a href="https://github.com/Perseus-Computing-LLC/perseus-vault/blob/main/benchmark/temporal/gauntlet_report.json">Temporal reconstruction report ↗</a><a href="{MEMCONFLICT_REPORT}" rel="noopener">Full eight-provider MemConflict report ↗</a></div></div></section>
<section class="section reproduce-section"><div class="wrap next-bar"><div><p class="kicker">Reproduce</p><h2>Start with the public source.</h2><p>The summary is sanitized. Raw prompts, conversations, credentials, and authorization headers are not published.</p></div><a class="button button-secondary" href="https://github.com/Perseus-Computing-LLC/perseus-vault">Open the Vault repository ↗</a></div></section>
"""
...
<section class="hero government-hero">
  <div class="wrap hero-layout">
    <div class="hero-copy">
      <p class="kicker">For DoD programs and primes</p>
      <h1>Local context and memory for AI-enabled work.</h1>
      <p class="lead">Perseus is an open-source software layer for current context, durable memory, and reviewable records around an approved model or workflow.</p>
      <div class="actions"><a class="button button-primary" href="{VAULT_DEMO}#try" rel="noopener">Try Vault demo ↗</a><a class="button button-secondary" href="{MEMCONFLICT_REPORT}" rel="noopener">Read the MemConflict report ↗</a><a class="text-link" href="/government/assets/Perseus-Computing-LLC-Capability-Statement.pdf">Capability brief →</a></div>
      <p class="boundary-note"><strong>Bounded workshare.</strong> Perseus supplies the software layer. The program retains mission authority; the prime or integrator owns interfaces, verification, and accreditation.</p>
    </div>
    <div class="hero-proof">
      <div class="proof-header"><span>Conflict handling / MemConflict</span><small>Self-run · 8 providers · 3,750 questions · measures reported separately</small></div>
      <div class="stat-grid">
        <article class="stat-card"><strong>0.555</strong><span>macro protocol score / -1 to +1</span><small>Eight self-hosted providers · self-run replication</small><a class="text-link" href="{MEMCONFLICT_REPORT}" rel="noopener">Read the full report ↗</a></article>
        <article class="stat-card risk"><strong>18</strong><span>wrong answers / 3,750 questions</span><small>Tested Perseus Vault configuration · wrong answers counted separately from blank responses</small><a class="text-link" href="{MEMCONFLICT_REPORT}" rel="noopener">See the outcome table ↗</a></article>
        <article class="stat-card wide"><strong>36.7%</strong><span>blank or abstaining / 1,378 of 3,750</span><small>Coverage trade-off in this self-run comparison · wrong answers counted separately</small><a class="text-link" href="{MEMCONFLICT_REPORT}" rel="noopener">Review the method ↗</a></article>
      </div>
      <p class="stat-footnote">This is a company-run replication, not official benchmark inclusion, customer validation, production validation, or Government authorization.</p>
    </div>
  </div>
</section>
<section class="section" id="fit"><div class="wrap"><div class="section-heading"><div><p class="kicker">Where it fits</p><h2>One bounded role beside the mission system.</h2></div><p>Perseus provides context, memory, and records. The program and its prime retain the surrounding mission responsibilities.</p></div><div class="role-grid"><article><span>Current context</span><h3>Start with the state of the work.</h3><p>Perseus Context Engine builds a briefing from the sources the host chooses.</p></article><article><span>Durable memory</span><h3>Carry decisions into the next task.</h3><p>Perseus Vault keeps dated facts, corrections, and choices locally.</p></article><article><span>Reviewable records</span><h3>Leave a trail people can check.</h3><p>Perseus Ledger records supplied events and references in a hash chain.</p></article></div></div></section>
<section class="section evidence-section" id="evidence"><div class="wrap proof-layout"><div><p class="kicker">Evidence</p><h2>Tested behavior. No certification claim.</h2><p>Each number has a method, a condition, and a limit. Read the reports before using a result in an evaluation or acquisition document.</p><div class="actions"><a class="button button-secondary" href="/benchmarks/">See all methods →</a><a class="text-link" href="{MEMCONFLICT_REPORT}" rel="noopener">Full MemConflict report ↗</a></div></div><div class="report-card"><span>MemConflict / tested Perseus Vault configuration</span><strong>18 wrong</strong><p>Across 3,750 questions in a self-run comparison of eight self-hosted configurations.</p><p class="card-note"><b>1,378 of 3,750 questions blank or abstaining (36.7%).</b> This is a conflict-handling result, not production validation.</p><a href="{MEMCONFLICT_REPORT}" rel="noopener">Read the eight-provider report ↗</a></div></div></section>
<section class="section boundary-section"><div class="wrap boundary-layout"><div><p class="kicker">Deployment boundary</p><h2>The program keeps the authority.</h2><p>Run the components locally, on premises, in a private cloud, or in a disconnected environment. The program chooses the data, keys, network, and permissions.</p><a class="text-link" href="/security/">See the security boundary →</a></div><dl class="boundary-table"><div><dt>Perseus</dt><dd>Current context, local memory, and reviewable records.</dd></div><div><dt>Program</dt><dd>Model choice, data classification, identity, authorization, network, safety, and mission authority.</dd></div><div><dt>Prime</dt><dd>Hardware, system interfaces, verification, validation, accreditation, and field support.</dd></div></dl></div></section>
<section class="section" id="evaluation"><div class="wrap"><div class="section-heading"><div><p class="kicker">Suggested evaluation</p><h2>Measure the failure modes that matter.</h2></div><p>A small, matched test tells the program more than a feature list.</p></div><div class="step-grid"><article><span>01</span><h3>Choose one workflow</h3><p>Define the task, data boundary, approved model, and deployment environment.</p></article><article><span>02</span><h3>Compare the paths</h3><p>Run a stateless baseline, ordinary retrieval, and Vault with the same task set and token budget.</p></article><article><span>03</span><h3>Review the result</h3><p>Record correctness, stale assertions, abstention, latency, recovery, access controls, and reviewable records.</p></article></div></div></section>
<section class="section next-section"><div class="wrap next-bar"><div><p class="kicker">Next step</p><h2>Start with the demo or the brief.</h2></div><div class="actions"><a class="button button-primary" href="{VAULT_DEMO}#try" rel="noopener">Try Vault demo ↗</a><a class="button button-secondary" href="/government/assets/Perseus-Computing-LLC-Capability-Statement.pdf">Download the brief</a><a class="button button-secondary" href="mailto:perseus@perseus.observer?subject=Perseus%20workflow">Talk through a workflow</a></div></div></section>
"""
...
PAGES = [
    ...
    ("vault/index.html", dict(path="/vault/", title="Perseus Vault · local memory for agent work", description="Store decisions, corrections, and dated facts locally so later work can find what matters.", current="platform", body=VAULT, page_class="product-page vault-page", cta_href=f"{VAULT_DEMO}#try", cta_label="Try demo ↗", schema_kind="product", code_repository=VAULT_SOURCE, og_image="/assets/og/perseus-vault.png")),
    ...
    ("benchmarks/index.html", dict(path="/benchmarks/", title="Perseus methods and evidence", description="See the headline Perseus measurements with their conditions, limitations, and linked source reports.", current="proof", body=BENCHMARKS_DETAIL, page_class="methods-page", cta_href=MEMCONFLICT_REPORT, cta_label="MemConflict ↗")),
    ...
    ("government/index.html", dict(path="/government/", title="Perseus for DoD programs and primes", description="See how Perseus can serve as a bounded local context, memory, and evidence component beside a mission system.", current="defense", body=GOVERNMENT, page_class="government-page", cta_href=f"{VAULT_DEMO}#try", cta_label="Try Vault demo ↗", og_image="/assets/og/government.png")),
...
REDIRECTS = {
    ...
    "services/index.html": ("Bounded workshare · Perseus", "/government/#evaluation", "Public engagement guidance now starts with a bounded workflow rather than a rate card."),
