...
- **`perseus scan` is faster on large contexts.** The secret/PII scanner masked
...
- **Mimir connector config-hash keyed on the `mimir` subtree only.** `_get_connector`
...
- **Connector merge diagnostics** — `merge_mneme_only` reported the mimir-only count
  instead of the local/mneme-only count (a copy of `merge_mimir_only`). Observability only.

### 🔒 Hardening

- **Mimir connector MCP stdio client is now genuinely fail-safe.** The connector is
