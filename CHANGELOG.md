...
- **#277** — `@memory` and `@mneme` cache fingerprints now include the active
  `mneme` connector configuration, so changing the Mneme command, transport, or
  backend settings invalidates stale cached memory output automatically.

### 🔗 Compatibility

- Updated Perseus defaults, doctor checks, init flow, bootstrap comments, and
  setup docs to launch separated Mneme directly as `mneme` / `mneme --db ...`
  instead of the legacy `mneme serve --mcp` form.

### 🔄 Rename

- **engram-rs → mneme** (#244) — The persistent memory backend is renamed from
  `engram-rs` to `mneme`. GitHub redirects automatically from the old
  `tcconnally/engram-rs` repo. All documentation, skills, config keys
  (`engram:` → `mneme:`), paths (`~/.perseus/engram/` → `~/.perseus/mneme/`),
  and class names (`EngramConnector` → `MnemeConnector`) updated throughout
  the docs and website. The binary is now `mneme` (was `engram`).
