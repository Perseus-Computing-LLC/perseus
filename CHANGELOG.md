...
- Fixed `Path.write_text` missing encoding on Windows — emoji (📌) crash in default prompts.
- Fixed `/bin/bash` unreachable on native Windows Python — added `_get_shell()` helper
  using `shutil.which()` with system-default fallback for both `@query` and `@services`.
- Fixed `@query` binary stdout NoneType crash — guarded `result.stdout` with `or ""`.
- Fixed `perseus --help` crash on Windows (Mnēmē macron `ē` can't encode to `cp1252`) —
  added `sys.stdout/stderr.reconfigure(encoding="utf-8")` at import time.

**New features:**

- **`render.max_query_bytes`** (default 256 KB) — caps runaway `@query` stdout with a
...
- **Configurable `@query` timeout** — `render.query_timeout_s` (default 30s) and per-directive
  `timeout=N` modifier (e.g. `@query "..." timeout=120`).
- **`render.parallel_services`** (opt-in, default off) — concurrent `@services` health checks
  via `ThreadPoolExecutor`. 100 services go from ~5 min serial to ~3 s parallel.
- **`render.parallel_queries`** (opt-in, default off) — pre-scans top-level `@query` directives
  and resolves them concurrently. Directives inside `@if` branches remain sequential.

**Integrations:**

- **VS Code / Cursor extension** — auto-renders on `.perseus/context.md` save, status bar
  indicator, auto-detects target assistant file, watch mode.
- **Claude Code session hook** — one `curl` install, runs `perseus render` before every
  Claude Code session.
- **GitHub Action** — renders context on push/schedule, commits back to repo so every
