...
- **feat** — `@bastra` directive: query persistent memories via the bastra-recall HTTP API. Args: `query="..."` (required), `scope="..."`, `k=N` (1–20, default 5), `type="..."`. Gracefully returns an info message if the daemon is unreachable.
- **feat** — `_bastra_recall()` HTTP client: lightweight POST to `/api/v1/recall` with 5-second timeout. Returns empty list on any failure — no exceptions propagate to the renderer.
- **feat** — `@memory` backend routing: set `memory.backend = "bastra"` in config to route the `@memory` directive through bastra-recall instead of the local Mnēmē file narrative. Focus modifiers (`decisions`, `patterns`, etc.) are mapped to bastra type filters automatically.
- **config** — Two new `memory` config keys: `backend` (`"file"` default | `"bastra"`) and `bastra_url` (default `http://127.0.0.1:6723`).
- **tests** — 20 new tests in `tests/test_bastra.py` covering `_bastra_recall()` (success, timeout, bad JSON, payload shape, custom URL) and `resolve_bastra()` / `resolve_memory()` backend routing.
