...
@query "npx vue --version" timeout=5 @cache ttl=300
...
@query "npx vue-tsc --version" timeout=5 @cache ttl=300
...
@query "ls -la src/composables/" timeout=5 @cache ttl=300
@query "ls -la src/store/" timeout=5 @cache ttl=300
@query "ls -la public/" timeout=5 @cache ttl=300
@query "wc -l src/**/*.{vue,ts,js} 2>/dev/null" timeout=5 @cache ttl=300
@query "cat package.json" timeout=5 @cache ttl=300
@query "cat tsconfig.json" timeout=5 @cache ttl=300
@query "cat nuxt.config.ts 2>/dev/null || cat vite.config.ts 2>/dev/null" timeout=5 @cache ttl=300
