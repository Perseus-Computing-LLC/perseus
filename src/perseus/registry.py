...
class DirectiveSpec(NamedTuple):
    ...
    is_semantic_hint: bool = False       # If True, the directive's value is a valid search hint for Vault
...
def _bind_registry() -> None:
    ...
    _entries: list[DirectiveSpec] = [
        ...
        DirectiveSpec("@skill-candidates", resolve_skill_candidates, ["status=", "limit="], "inline", "ac", reads_files=True, cacheable=True, summary="List mined procedural-skill candidates (from session transcripts, #932) pending operator review: trigger, steps, pitfalls, evidence sessions, token cost. Candidates are staged, never active — `perseus skills approve <name>` promotes one to the live skills dir (@skills). Opt-in surfacing: place the directive in your context source; the mining pipeline never writes AGENTS.md/CLAUDE.md. Read-only.", tier=2),
        ...
        DirectiveSpec("@drift",     resolve_drift,     [],                         "inline",  "ac",  reads_files=True, summary="Detect drift between predicted and actual tool usage patterns via the Guide. Use when tool behavior seems off or after config changes. For workspace hygiene checks, prefer perseus_health. Read-only; returns a markdown drift report.", tier=2),
        ...
        DirectiveSpec("@vault",    resolve_vault,    ["query=", "scope=", "k=", "type="], "inline", "acw", safe_for_hover=True, summary="Query Perseus Vault for scoped, durable context. Read-only; falls back to the local Vault FTS5 index when the service is unavailable.", tier=2, is_semantic_hint=True),
