...
def test_launchd_subcommand_scaffolds_plist_on_macos(tmp_path, monkeypatch):
    ...
    assert plist.exists()
    plist_body = plist.read_text(encoding="utf-8")
...
def test_load_config_prefers_perseus_env_vars(monkeypatch):
    ...
    # Compare Path objects, not str: on Windows str(Path('/tmp/x')) is
    # '\\tmp\\x', but Path equality is platform-normalized.
    assert mod.SKILLS_DIR == Path('/tmp/perseus-skills')
    assert mod.SESSIONS_DIR == Path('/tmp/perseus-sessions')
...
def test_pack_manifest_validation_accepts_profile_pack(tmp_path):
    ...
    (tmp_path / ".perseus" / "pack.yaml").write_text(yaml.safe_dump(manifest), encoding="utf-8")
...
def test_init_profile_generic_writes_context_and_pack(tmp_path, capsys):
    ...
    assert pack.exists()
    assert str(tmp_path) not in context.read_text(encoding="utf-8")
    manifest = yaml.safe_load(pack.read_text(encoding="utf-8"))
...
def test_init_profile_with_output_override(tmp_path, capsys):
    ...
    manifest = yaml.safe_load((tmp_path / ".perseus" / "pack.yaml").read_text(encoding="utf-8"))
