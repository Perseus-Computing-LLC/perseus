#!/usr/bin/env python3
"""Apply fixes for #103 (_walk_dot_path) and #109 (@date relative dates) to misc.py"""
import re
...
# ── #103: _walk_dot_path list index support ──
old_walk = '''def _walk_dot_path(obj: object, dot: str) -> object:
    cur = obj
    if not dot:
        return cur
...
assert old_walk in content, "old _walk_dot_path not found"
content = content.replace(old_walk, new_walk)
print("OK #103: _walk_dot_path list index support")

# ── #109: @date relative dates ──
old_date = '''def resolve_date(args_str: str) -> str:
...
new_date = '''def resolve_date(args_str: str) -> str:
    ...
    # Parse offset and days-ago from remaining args (strip format=... first)
...
assert old_date in content, "old resolve_date not found"
content = content.replace(old_date, new_date)
print("OK #109: @date relative dates (offset, days-ago)")
