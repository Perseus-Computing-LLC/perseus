...
def seed():
    ...
    components = [
        ...
        {"name": "vault_connector", "status": "active", "owner": "tcconnally", "test_coverage": "79%", "last_modified": "2026-06-07", "dependencies": ["perseus-vault"], "description": "Perseus Vault connector — canonical MCP retrieval over structured entities and FTS5"},
        {"name": "vault_index", "status": "active", "owner": "tcconnally", "test_coverage": "88%", "last_modified": "2026-06-07", "dependencies": ["perseus-vault"], "description": "Perseus Vault index maintenance — FTS5 schema and retrieval health"},
        {"name": "memory_mesh", "status": "experimental", "owner": "contributor-01", "test_coverage": "34%", "last_modified": "2026-06-01", "dependencies": ["vault_connector.py", "sibyl_memory.py"], "description": "Cross-workspace memory federation — aggregates Vault + Sibyl results"},
        ...
        {"name": "test_vault_retrieval", "status": "active", "owner": "tcconnally", "test_coverage": "84%", "last_modified": "2026-06-07", "dependencies": ["pytest", "vault"], "description": "Vault retrieval tests — FTS5 accuracy benchmarks"},
        {"name": "test_vault_stability", "status": "active", "owner": "tcconnally", "test_coverage": "81%", "last_modified": "2026-06-07", "dependencies": ["pytest", "vault"], "description": "Vault stability tests — concurrent read/write patterns"},
        {"name": "test_vault_efficiency", "status": "active", "owner": "tcconnally", "test_coverage": "77%", "last_modified": "2026-06-07", "dependencies": ["pytest", "vault"], "description": "Vault token efficiency benchmarks"},
    ...
    decisions = [
        ...
        {"decision": "Graceful degradation for all integrations", "date": "2026-05-29", "status": "active", "rationale": "If Sibyl/Vault/MCP is absent or errors, return empty string. Never crash AGENTS.md generation.", "alternatives_considered": ["Hard dependency (rejected: breaks adopters without Sibyl)", "Feature flags (rejected: added complexity)"], "impacted_components": ["sibyl_memory.py", "vault_connector.py", "mcp.py"]},
        ...
        {"decision": "Cross-workspace memory federation via Vault", "date": "2026-05-26", "status": "active", "rationale": "Different workspaces accumulate different memories. Vault federation allows cross-workspace recall.", "alternatives_considered": ["Single global DB (rejected: namespace collisions)", "Sibyl-only federation (rejected: Vault covers semantic search Sibyl doesn't)"], "impacted_components": ["memory_mesh.py", "vault_connector.py"]},
        ...
        {"decision": "engram-rs renamed to Vault", "date": "2026-05-30", "status": "active", "rationale": "Branding clarity. Binary renamed from engram to vault. DB path: .minions-data/vault/.", "alternatives_considered": ["Keep engram-rs name (rejected: confusing with 'engram' concept in neuroscience)", "Rename to Hermes-Memory (rejected: too coupled to Hermes)"], "impacted_components": ["vault_connector.py", "memory_mesh.py"]},
        ...
        {"decision": "Perseus Vault is the canonical persistent-memory backend", "date": "2026-05-30", "status": "active", "rationale": "The Vault connector provides structured entities, FTS5 retrieval, and durable provenance through the canonical MCP surface.", "alternatives_considered": ["Multiple memory providers (rejected: split authority)", "Local-only retrieval (rejected: no durable cross-session substrate)"], "impacted_components": ["vault_connector.py", "vault_index.py"]},
        ...
        {"decision": "No flat files for memory", "date": "2026-05-28", "status": "active", "rationale": ".txt/.json/.csv/.md memory dumps are brittle, unsearchable, and prone to version conflicts. Use Vault or Sibyl.", "alternatives_considered": ["Allow flat files (rejected: agents create litter)", "Ban all file I/O (rejected: too restrictive)"], "impacted_components": ["convention_checker.py"]},
    ...
    bugs = [
        ...
        {"title": "Vault FTS5 search escaping bug on special chars", "severity": "high", "component": "vault_connector", "status": "fixed", "reported": "2026-05-28", "reproduction": "Search query containing ':' or '@' causes FTS5 syntax error", "assigned_to": "tcconnally", "related_issue": "#287", "fixed_in": "v1.0.6"},
        {"title": "Stale Vault index after rapid writes", "severity": "medium", "component": "vault_connector", "status": "fixed", "reported": "2026-05-29", "reproduction": "Write 50 memories, search immediately — some don't appear in results", "assigned_to": "tcconnally", "related_issue": "#290", "fixed_in": "v1.0.6"},
        ...
        {"title": "Memory mesh duplicates results across Vault + Sibyl", "severity": "low", "component": "memory_mesh", "status": "open", "reported": "2026-06-04", "reproduction": "Same fact stored in both Perseus Vault and Sibyl entity — mesh returns it twice", "assigned_to": "contributor-01", "related_issue": "#311"},
    ...
    conventions = [
        ...
        {"rule": "No flat files for memory", "scope": "All agents in this workspace", "rationale": ".txt/.json/.csv/.md memory dumps are unsearchable and cause version conflicts.", "examples": ["DO: use Vault (MCP) or Sibyl Memory for durable storage", "DON'T: write 'remember-these-facts.json' to disk"]},
    ...
    infra = [
        ...
        {"type": "database", "name": "Perseus Vault", "path": ".minions-data/vault/", "engine": "SQLite + FTS5", "role": "Semantic search over markdown vaults"},
    ...
    # ═══════════════════════════════════════════════════════════════
    sessions = [
        {"title": "Fix Vault FTS5 escaping and stale-index bugs", "date": "2026-05-28", "decisions_made": ["Escaped special chars in FTS5 queries", "Added index warming after rapid writes"], "files_changed": ["src/perseus/vault_connector.py"], "outcome": "Fixed in v1.0.6"},
        ...
        {"title": "Memory hygiene audit", "date": "2026-06-07", "decisions_made": ["Cleaned 4 stale memory entries", "Offloaded 2 to Perseus Vault"], "files_changed": ["memory"], "outcome": "Memory store at 80% capacity — healthy"},
        ...
        {"title": "Benchmark gauntlet: memory connector comparison", "date": "2026-05-30", "decisions_made": ["Vault vs Sibyl vs flat files measured", "Sibyl wins on retrieval precision, Vault on semantic breadth"], "files_changed": ["benchmark/gauntlet/"], "outcome": "Gauntlet results published"},
    ]
    ...
    references = [
        ...
        {"title": "Vault Memory Connector Reference", "path": "docs/vault-connector.md", "audience": "Developers", "topics": ["FTS5 syntax", "Vault structure", "Federation setup"]},
    ...
    journal_events = [
        ...
        {"evaluated": "Set up memory connector comparison: Vault vs Sibyl vs flat files", "acted": "Built test harness with 50 retrieval queries per backend", "forward": "Running benchmarks...", "extra": {"session": "gauntlet", "turn": 1}},
        {"evaluated": "Results: Sibyl 50/50 retrieval, Vault 47/50, flat files 31/50", "acted": "Wrote gauntlet report with per-query breakdown", "forward": "Report published at benchmark/gauntlet/gauntlet_report.md", "extra": {"session": "gauntlet", "turn": 2}},
        ...
        {"evaluated": "Offloaded 2 entries to Perseus Vault, removed 2 completely stale", "acted": "Updated memory store, now at 78% capacity", "forward": "Memory hygiene maintained. Set 80% alert threshold.", "extra": {"session": "memory-hygiene", "turn": 2}},
