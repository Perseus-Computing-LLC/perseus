...
| Perseus Vault | complete | Narrative project memory and query. v1.0.6 adds optional Perseus Vault hybrid accelerator (project Synapse) — MCP-based remote memory with Ebbinghaus time-decay and semantic+BM25 hybrid search; circuit-breaker protected, degrades to local-only. |
