...
`forget`, `correct`, `context`, `status`, and `health`). The full registry remains
available under the default/all profile. Evaluate the installed binary and the
MCP `initialize`/`tools/list`/`tools/call` flow rather than relying on a registry
count copied from another release.
...
provenance layer for actions and resource/accounting events; it is not the
memory store and does not replace the Context Engine or Vault.
