...
| M3 | **Ship signed releases with SLSA provenance** | Complete and enable it, so users can verify the artifact they install. | 🟡 In progress — GitHub Artifact Attestations over sdist/wheel + PEP 740 PyPI attestations wired into `publish.yml`; validates on the next release. |
