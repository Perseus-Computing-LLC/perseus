...
The Perseus Gauntlet (`benchmark/gauntlet/`) is the ultimate enterprise torture test — 11 phases, 25 role profiles, 40+ certification gates. It requires Linux (the orchestrator uses Linux-specific filesystem features for adversarial scenarios). Smoke mode takes ~20 minutes; full mode takes ~6–8 hours.
