# NIST AI RMF Alignment — Perseus & Perseus Vault

> Mapping Perseus (live context engine) and Perseus Vault (persistent memory) to the
...
Map, Measure, Manage.** This document maps Perseus and Perseus Vault capabilities to
...
| **GOVERN 1.2** — AI system transparency and accountability mechanisms | Perseus traces every context injection to source (file, line, timestamp). Perseus Vault journal provides immutable decision provenance. |
| **GOVERN 1.4** — Organizational risk tolerance for AI is established | Both systems are read-only by default. Perseus never writes to filesystem. Perseus Vault's connectors are opt-in. Explicit trust boundaries documented. |
...
| **GOVERN 2.3** — AI system inventory maintained and accessible | Perseus Vault's entity model provides typed, searchable inventory of all knowledge assets. Perseus's context snapshots serve as an operational inventory. |
| **GOVERN 3.1** — Decision-making accountability assigned | Perseus's decision trace maps agent actions to specific context state. Perseus Vault's journal records who touched what and when. |
...
| **GOVERN 5.1** — AI risk management integrated with cybersecurity | AES-256-GCM encryption (Perseus Vault). Zero network by default (both). CMMC Level 2 readiness (Perseus Vault). Attack surface analysis documented. |
...
| AI RMF Subcategory | How We Address It |
|---|---|
| **MAP 1.1** — Intended purpose and deployment context understood | Perseus is a read-only context engine — explicitly not a decision-maker. Perseus Vault stores agent memory — explicitly not a model. Deployment context: local-first, air-gap ready, MCP-native. |
| **MAP 1.2** — AI system's tasks and expected behavior defined | Perseus: resolve directives → render markdown → inject. Perseus Vault: store/recall/search entities → respond via MCP JSON-RPC. Both documented with input/output contracts. |
| **MAP 1.3** — AI system's knowledge limits documented | Perseus renders only what directives resolve — no hallucination. Perseus Vault searches only stored entities — no generative capacity. Both systems are "dumb infrastructure" — they provide ground truth, not opinions. |
| **MAP 1.4** — AI system's outputs and decisions characterized | Perseus output is deterministic markdown. Perseus Vault output is structured JSON. Both are versioned, reproducible, and auditable. |
...
| **MAP 3.2** — Human-AI configuration and oversight delineated | Perseus: human authors directives, engine resolves them, assistant reads output. Perseus Vault: human configures encryption, agent uses memory, audit trail is human-readable. Clear boundaries at every interface. |
| **MAP 4.1** — Approaches to measuring trustworthiness identified | Perseus: context freshness (TTL), directive resolution time, token compression ratio. Perseus Vault: entity recall precision, journal integrity (hash chain), decay scoring. Gauntlet v2 benchmark: 100.0/100 score. |
...
| AI RMF Subcategory | How We Address It |
|---|---|
| **MEASURE 1.1** — Trustworthiness characteristics tested | Perseus: 1,032 tests in CI. Perseus Vault: Rust test suite + demo smoke tests. Both: deterministic output testing. |
| **MEASURE 1.2** — System performance, safety, security assessed | Perseus: 450x cold→warm speedup, 94% token compression, 0 failures at 150 concurrent writes. Perseus Vault: sub-ms SQLite queries, FTS5 + hybrid vector search. Security: attack surface analysis in SECURITY.md. |
| **MEASURE 2.1** — AI system functionality and behavior monitored | Perseus context snapshots provide continuous monitoring of operational state. Perseus Vault journal provides append-only operation log. Both: CI on every push. |
...
| **MEASURE 2.5** — AI system evaluated for privacy risks | Local-first: no data leaves the deployment boundary. No telemetry. No analytics. No cloud dependency. Memory encryption at rest. Workspace isolation in Perseus Vault. |
...
| **MEASURE 2.7** — Explainability, interpretability, and transparency evaluated | Perseus: full context provenance (file, line, timestamp). Perseus Vault: typed entities with relationship graph and journal trail. PROV-O export (in development). Decision trace dashboard (in development). |
| **MEASURE 2.8** — AI system evaluated for environmental impact | Perseus: single Python process, no GPU. Perseus Vault: single Rust binary, 10MB footprint, bundled SQLite. Both: minimal compute requirements. |
...
| AI RMF Subcategory | How We Address It |
|---|---|
| **MANAGE 1.1** — AI risks prioritized and responded to | Public issue trackers for both repos. Security vulnerability reporting process (perseus@perseus.observer, 48-hour response). Correction capture (Perseus Vault: mimir_correct tool). |
...
| **MANAGE 2.2** — AI system decommissioned per policy | Not applicable (both are pure infrastructure — no persistent state beyond user's database/context files). Perseus Vault database is single-file SQLite for easy archival. |
