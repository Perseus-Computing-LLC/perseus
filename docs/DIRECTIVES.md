...
| `@sibyl [query=\"topic\"] [tiers=entity,state]` | **Automatic injection** — Sibyl Memory context is injected by the render pipeline when `SIBYL_MEMORY_ENABLED=1`. Not a resolved directive; the line in context.md is an informational placeholder. Degrades gracefully to empty when absent. |
