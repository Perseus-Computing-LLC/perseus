...
| `@skill-candidates [status=pending] [limit=N]` | Renders mined procedural-skill candidates (from session transcripts, #932): name, description, trigger, evidence sessions, token cost. Candidates are **staged, never active** — `perseus skills approve <name>` is the only activation path (review gate). Opt-in surfacing: place this directive in your context source; the mining pipeline never writes AGENTS.md/CLAUDE.md itself. `status=all|pending|approved|rejected` filters; each render records a #929-line telemetry event measuring summary-vs-full-bodies token impact. |
